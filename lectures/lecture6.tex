...
%				
		...
		\subsubsection*{Entscheidungsgrenze:}
			...
			\noindent Wie sieht die Entscheidungsgrenze Geometrisch aus? \\
			Fall $ \Theta_0 = 0: x: \Theta^Tx = 0$ \quad \footnote{D.h. x muss senkrecht zum Vektor $ \Theta $ sein $ \Rightarrow $ Entscheidungsgrenze ist ein $ n-1 $- Dimensionaler Unterraum des $ \mathbb{R}^n $}
...
%			\end{figure}
			...
			\noindent [Im ganz allgemeinen Modell haben wir $ 2^{n+1}-1 $ Parameter]
